/**
 * storage.js — Persistência no Drive (banco de dados via Google Sheets + Apps Script).
 *
 * O "banco" é uma planilha no Drive da 4blue. O Apps Script publicado como Web App
 * recebe as chamadas deste arquivo e grava/lê as linhas. Um registro = um carrossel
 * (nome, autor, data e o texto do editor). Assim a equipe reabre e edita depois.
 *
 * Configuração: cole a URL /exec do Web App em APPS_SCRIPT_URL (ver app/README.md).
...
function semBackend() {
  if (!APPS_SCRIPT_URL) {
    setStatus("Configure a URL do Apps Script em storage.js para salvar no Drive.", "erro");
    return true;
  }
  return false;
}
...
function nomeSugerido(texto) {
  // Usa a primeira linha significativa como nome sugerido.
...
async function publicarNoDrive() {
  ...
  setStatus("Renderizando os cards e enviando ao Drive... isso pode levar alguns segundos.");
  try {
    // Renderiza cada card em PNG 4× (mesma escala do download local).
    const imagens = [];
    for (let i = 0; i < currentSlides.length; i++) {
      ...
      const canvas = await html2canvas(el, {
        scale: 4, useCORS: true, backgroundColor: bg,
    ...
    const resposta = await fetch(APPS_SCRIPT_URL, {
      ...
      body: JSON.stringify({ acao: "publicar_png", id: carrosselAtualId, nome: nome, imagens: imagens })
    });
    const dados = await resposta.json();
    if (dados.ok) {
      const el = document.getElementById("status-app");
...
async function abrirBiblioteca() {
  ...
  try {
    ...
    for (const item of dados.itens) {
      ...
      div.innerHTML =
        '<div><div class="s-nome"></div><div class="s-meta"></div></div>' +
        '<span class="btn-dl" style="width:auto">Abrir</span>';
...
async function abrirCarrossel(id) {
  ...
  try {
    ...
    fecharBiblioteca();
    if (typeof generateCarousel === "function") generateCarousel();
